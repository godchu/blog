...
import { SandpackStack, useSandpack } from '@codesandbox/sandpack-react/unstyled';
...
export function Preview({ isExpanded, className, lintErrors }) {
  const { sandpack, listen } = useSandpack();
  const [bundlerIsReady, setBundlerIsReady] = useState(false);
  const [showLoading, setShowLoading] = useState(false);
  const [iframeComputedHeight, setComputedAutoHeight] = useState(null);
        ...
        <div style={iframeWrapperPosition()}>
          ...
          <iframe
            ...
            className={cn(
              ...
              hideContent ? 'absolute opacity-0 pointer-events-none duration-75' : 'opacity-100 duration-150',
            )}
