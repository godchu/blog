...
import { Fragment, useCallback, useEffect, useInsertionEffect, useRef, useState } from 'react';
import { FileTabs, useSandpack, useSandpackNavigation } from '@codesandbox/sandpack-react/unstyled';
...
import { OpenInTypeScriptPlaygroundButton } from './open-in-type-script-playground';
...
export function NavigationBar({ providedFiles }) {
    ...
    <div className="bg-wash dark:bg-card-dark flex justify-between items-center relative z-10 border-b border-border dark:border-border-dark rounded-t-lg text-lg">
      ...
      <div className="flex-1 grow min-w-0 px-4 lg:px-6">
        ...
        <Listbox value={activeFile} onChange={setActiveFile}>
          ...
          {isMultiFile && showDropdown && (<Listbox.Options className="absolute mt-0.5 bg-card dark:bg-card-dark px-2 inset-x-0 mx-0 rounded-b-lg border-1 border-border dark:border-border-dark rounded-sm shadow-md">
              {/* @ts-ignore: the Listbox type from '@headlessui/react' is incompatible with JSX in React 19 */}
              {visibleFiles.map((filePath) => (<Listbox.Option key={filePath} value={filePath} as={Fragment}>
                  {({active}) => (
                    <li
                      className={cn(
      ...
      <div className="px-3 flex items-center justify-end text-start" translate="yes">
        ...
        {activeFile.endsWith('.tsx') && (
          <OpenInTypeScriptPlaygroundButton content={sandpack.files[activeFile]?.code || ''} />
        )}
      </div>
    </div>
  );
