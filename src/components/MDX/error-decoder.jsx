...
import { useErrorDecoderParams } from '../error-decoder-context';
...
function urlify(str) {
  ...
  return segments.map((message, i) => {
    if (i % 2 === 1) {
      return (
        <a key={i} target="_blank" className="underline" rel="noopener noreferrer" href={message}>
          {message}
        </a>
      );
    }
...
export default function ErrorDecoder() {
  ...
  const [message, setMessage] = useState(() => (errorMessage ? urlify(errorMessage) : null));
