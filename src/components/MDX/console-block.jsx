...
const Box = ({ width = '60px', height = '17px', className, customStyles }) => (
  <div className={className} style={{ width, height, ...customStyles }} />
);
...
export function ConsoleBlock({ level = 'error', children }) {
  ...
  return (
    <div className="console-block mb-4 text-secondary bg-wash dark:bg-wash-dark rounded-lg" translate="no" dir="ltr">
      ...
      <div
        className={cn('flex px-4 pt-4 pb-6 items-center content-center font-mono text-code rounded-b-md', {
...
export function ConsoleBlockMulti({ children }) {
  return (
    <div className="console-block mb-4 text-secondary bg-wash dark:bg-wash-dark rounded-lg" translate="no" dir="ltr">
      ...
      <div className="grid grid-cols-1 divide-y divide-gray-300 dark:divide-gray-70 text-base">{children}</div>
    </div>
  );
}

export function ConsoleLogLine({ children, level }) {
  let message;
  if (typeof children === 'string') {
    message = children;
  } else if (isValidElement(children)) {
  ...
  return (
    <div
      className={cn('ps-4 pe-2 pt-1 pb-2 grid grid-cols-[18px_auto] font-mono rounded-b-md', {
        'bg-red-30 text-red-50 dark:text-red-30 bg-opacity-5': level === 'error',
        'bg-yellow-5 text-yellow-50': level === 'warning',
        'bg-gray-5 text-secondary dark:text-secondary-dark': level === 'info',
      })}
    >
      {level === 'error' && <IconError className="self-start mt-1.5 text-[.7rem] w-6" />}
      {level === 'warning' && <IconWarning className="self-start mt-1 text-[.65rem] w-6" />}
      <div className="px-2 pt-1 whitespace-break-spaces text-code leading-tight">{message}</div>
    </div>
  );
}
