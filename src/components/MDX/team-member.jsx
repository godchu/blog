...
import { IconBsky } from '../icon/icon-bsky';
import { IconGitHub } from '../icon/icon-gitHub';
import { IconLink } from '../icon/icon-link';
import { IconThreads } from '../icon/icon-threads';
...
export function TeamMember({ name, title, permalink, children, photo, github, twitter, threads, bsky, personal }) {
  if (name === null || title === null || permalink === null || children === null) {
    const identifier = name ?? title ?? permalink ?? 'unknown';
    throw new Error(`Expected name, title, permalink, and children for ${identifier}`);
  }
  return (
    <div className="pb-6 sm:pb-10">
      <div className="flex flex-col sm:flex-row height-auto">
        <div className="hidden sm:block basis-2/5 rounded overflow-hidden relative" style={{ width: 300, height: 250 }}>
