...
export function SidebarRouteTree({ isForceExpanded, breadcrumbs, routeTree, level = 0 }) {
  const slug = usePathname().split(/[\?\#]/)[0];
  const pendingRoute = usePendingRoute();
  const currentRoutes = routeTree.routes;
  return (
    <ul>
      {currentRoutes.map(({ path, title, routes, version, heading, hasSectionHeader, sectionHeader }, index) => {
        ...
        if (!path || heading) {
          ...
          const isBreadcrumb = breadcrumbs.length > 1 && breadcrumbs[breadcrumbs.length - 1].path === path;
        ...
        if (hasSectionHeader) {
          let sectionHeaderText =
            sectionHeader !== undefined ? sectionHeader.replace('{{version}}', siteConfig.version) : '';
          return (
            <Fragment key={`${sectionHeaderText}-${level}-separator`}>
              {index !== 0 && (
                <li role="separator" className="mt-4 mb-2 ms-5 border-b border-border dark:border-border-dark" />
              )}
              <h3
                className={cn(
