...
export const DocsPageFooter = memo(({ nextRoute, prevRoute, route }) => {
    ...
    <>
      {prevRoute?.path || nextRoute?.path ? (
        <div className="grid grid-cols-1 gap-4 py-4 mx-auto max-w-7xl md:grid-cols-2 md:py-12">
          {prevRoute?.path ? <FooterLink type="Previous" title={prevRoute.title} href={prevRoute.path} /> : <div />}

          {nextRoute?.path ? <FooterLink type="Next" title={nextRoute.title} href={nextRoute.path} /> : <div />}
        </div>
      ) : null}
    </>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col overflow-hidden">
        ...
        <span className="text-lg break-words group-hover:underline">{title}</span>
      </div>
    </NextLink>
  );
}
